...
class IAMUser(BaseModel):
    name: str
    arn: str
    uid: str = Field(default_factory=lambda data: get_guid(data["name"], scope="aws", kube_type="iam-user", name=data["name"]))
    groups: list[str]
...
class IAMUserNode(Node):
    ...
    def from_input(cls, **kwargs) -> "IAMUserNode":
        ...
        properties = ExtendedProperties(name=model.name, displayname=model.name, objectid=model.uid, groups=model.groups)
        return cls(id=model.uid, kinds=["AWSIAMUser", "KubeUser"], properties=properties)
