...
class LookupManager:
    ...
    def nodes(self, name: str) -> str:
        self.con.execute(f"SELECT metadata.uid FROM {self.db}.nodes WHERE metadata.name = ?", [name])
        results = self.con.fetchone()
        return str(results[0]) if results else ""

    def custom_resource_definitions(self, resource: str) -> str:
        self.con.execute(f"SELECT uid FROM {self.db}.custom_resource_definitions WHERE name = ?", [resource])
        results = self.con.fetchone()
        return str(results[0]) if results else ""

    def resource_definitions(self, resource: str) -> str:
        self.con.execute(f"SELECT uid FROM {self.db}.resource_definitions WHERE name = ?", [resource])
        results = self.con.fetchone()
        return str(results[0]) if results else ""

    def service_accounts(self, name: str, namespace: str) -> str:
        self.con.execute(f"SELECT metadata.uid FROM {self.db}.service_accounts WHERE metadata.name = ? AND metadata.namespace = ?", [name, namespace])
        results = self.con.fetchone()
        return str(results[0]) if results else ""

    def roles(self, name: str, namespace: str) -> str:
        self.con.execute(f"SELECT metadata.uid FROM {self.db}.roles WHERE metadata.name = ? AND metadata.namespace = ?", [name, namespace])
        results = self.con.fetchone()
        return str(results[0]) if results else ""

    def cluster_roles(self, name: str) -> str:
        self.con.execute(f"SELECT metadata.uid FROM {self.db}.cluster_roles WHERE metadata.name = ?", [name])
        results = self.con.fetchone()
        return str(results[0]) if results else ""


    def namespaces(self, name) -> str:
        self.con.execute(f"SELECT metadata.uid FROM {self.db}.namespaces WHERE metadata.name = ?", [name])
        results = self.con.fetchone()
        return str(results[0]) if results else ""
